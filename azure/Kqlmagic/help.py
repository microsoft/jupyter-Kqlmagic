...
* Azure CLI - Use your active Azure CLI sessions for authentication.
* anonymous - No authentication. For the case that you run your data source locally.


Azure CLI authentication requires an active session (```az login```) on the machine or container the Python kernel is running on.<br>
- If multiple logins exist in your session, the Azure CLI default subscription context (```az account show```) will be used. Subscription contexts are associated with a single identity. List subscriptions and their logged in identities with ``az account list``.<br>
- ```subscription='<subscription-id>'``` can be added to the connection string to select a subscription context (equivilant to ```--subscription``` in the Azure CLI).<br>
...
```%kql azuredataexplorer://azcli;cluster='<cluster-name>';database='<database-name>';alias='<database-friendly-name>'```<br><br>
...
```%kql loganalytics://azcli;workspace='<workspace-id>';alias='<workspace-friendly-name>'```<br><br>
...
```%kql appinsights://azcli;appid='<app-id>';alias='<appid-friendly-name>'```<br><br>
