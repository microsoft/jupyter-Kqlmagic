        ...
        if load_mode != "silent":
            ...
            try:
                ...
                if pypi_version and compare_version(pypi_version) > 0:
                    ...
                    #Display What's new  button #
                    file_path = "https://raw.githubusercontent.com/microsoft/jupyter-Kqlmagic/master/HISTORY.txt" #need to merge HISTORY.txt file for this to work
                    data = urllib.request.urlopen(file_path)
                    data_decoded = data.read().decode('utf-8')
                    data_as_markdown = MarkdownString(data_decoded)
                    Display.show_window_with_markdown_str("Whats New", button_text="What's New?", onclick_visibility="visible", markdown_str=data_as_markdown)
